module Exfm
  ...
  def self.search(input, results)
    ...
    @exfm_songs = []
    tracks.each do |t|
      artist = Artist.find_or_create_by_name(:name => t["artist"]);
      newSong = Song.find_or_create_by_title_and_audio_and_image(:title => t["title"], :audio => t["url"], :image => t["image"]["large"])
